...
type Customer = { name: string; whatsapp: string };
...
export default function App(): JSX.Element {
  const pizzas = PIZZAS as PizzaItem[];
  ...
  const mostOrderedScrollRef = useRef<HTMLDivElement | null>(null);
  const cartScrollRef = useRef<HTMLDivElement | null>(null);
  const mostOrderedCardRefs = useRef<Array<HTMLDivElement | null>>([]);
  const mostOrderedRafRef = useRef<number>(0);
  const [mostOrderedActiveIndex, setMostOrderedActiveIndex] = useState(0);
  const [mostOrderedHoverIndex, setMostOrderedHoverIndex] = useState<number | null>(
    null
  );
  const promoMetaRef = useRef<{ lastShownCode: string; lastSubtotal: number }>({
    lastShownCode: "",
    lastSubtotal: 0,
  });
  const { items, addItem, incrementItem, decrementItem, removeItem, total } = useCart();
  ...
  const mapsEmbedUrl = `https://www.google.com/maps?q=${encodeURIComponent(destination)}&z=17&output=embed`;
  ...
  const openPromotion = (offer: PromoOffer | null): void => {
    if (!offer) return;
    setPromoOffer(offer);
    setPromoOpen(true);
  ...
  const findDrinkCartItem = (drinkName: string) =>
    (items as unknown as CartItem[]).find(
      (it) =>
        it?.sizeKey === "U" &&
        it?.name === drinkName &&
  ...
  const mostOrderedMeta = [
    ...
    },
    {
      ...
      description: "Pizza feita com banana, mussarela e chocolate ao leite.",
    },
    {
      name: "Banca",
      description: "Pizza feita com mussarela, banana, bacon e melado de cana.",
    },
  ];
  ...
  const mostOrderedByName = new Map(mostOrderedMeta.map((p) => [p.name, p.description]));

  const mostOrderedPizzas = mostOrderedMeta
    .map((m) => pizzas.find((p) => p.name === m.name))
    .filter((p): p is PizzaItem => Boolean(p));
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "url(/back3.webp?v=1)",
    ...
    >
          ...
          <div className="mb-10">
            ...
            <div className="flex justify-center gap-1.5 mb-3">
              {[0, 1, 2, 3, 4].map((i) => (
                ...
                >
                  <defs>
                    <linearGradient id={`introStarGrad-${i}`} x1="0" y1="0" x2="1" y2="1">
                      <stop offset="0" stopColor="#fff1b8" />
                      <stop offset="0.45" stopColor="#facc15" />
                      <stop offset="1" stopColor="#f59e0b" />
                    </linearGradient>
                    <linearGradient id={`introStarShine-${i}`} x1="0" y1="0" x2="0" y2="1">
          ...
          <div className="rounded-2xl bg-white/80 border border-red-500/20 p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h3 className="text-lg font-black text-black mt-1">AS MAIS PEDIDAS</h3>
              </div>
                ...
                <div className="relative rounded-2xl overflow-hidden border border-red-500/20 bg-white shadow-sm">
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-black font-black text-sm truncate">Sitári Pizzaria</p>
            ...
            <div className="rounded-2xl bg-white/80 border border-red-500/20 p-6 shadow-sm">
              ...
              <div className="flex items-center gap-0.5 mt-3">
                {[1, 2, 3, 4, 5].map((i) => (
                  ...
                  >
                    <defs>
                      <linearGradient id={`reviewStarGrad-${i}`} x1="0" y1="0" x2="1" y2="1">
                        <stop offset="0%" stopColor="#fff7c2" />
                        <stop offset="35%" stopColor="#fde68a" />
                        <stop offset="65%" stopColor="#fbbf24" />
                        <stop offset="100%" stopColor="#b45309" />
                      </linearGradient>
                      <linearGradient id={`reviewStarShine-${i}`} x1="0" y1="0" x2="0" y2="1">
      ...
      <AnimatePresence>
        {(items as unknown[]).length > 0 && !cartOpen && (
          <motion.div
            initial={{ y: 100 }}
            animate={{ y: 0 }}
            exit={{ y: 100 }}
          ...
          >
            ...
            >
              ...
              <span className="relative font-black text-lg">R$ {total.toFixed(2)}</span>
            </button>
          </motion.div>
        )}
      </AnimatePresence>
                  ...
                  <>
                    <div className="p-4 space-y-3">
                      {drinkOptions.length > 0 && (
                        <div className="rounded-2xl bg-white/80 border border-red-500/20 p-3 shadow-sm">
                          ...
                          <h4 className="text-black font-black text-sm mt-1">Adicionar bebidas</h4>
                      ...
                      {(items as unknown as CartItem[]).map((item) => {
                          ...
                          >
                            <div className="flex gap-4 items-center min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                {Array.isArray(item.flavors) && item.flavors.length > 1 && (
                                  <p className="text-[10px] text-black/50 whitespace-normal break-words leading-tight mt-1">
                                    Sabores: {item.flavors.join(" / ")}
                                  </p>
                                )}
                                <p className="text-[10px] text-black/50">Tamanho {item.size}</p>
                                <p className="text-[10px] text-black/50">Quantidade {qty}</p>
                              </div>
                            </div>
                    ...
                    <div className="p-6 bg-white border-t border-red-500/20 space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-black text-3xl font-black">R$ {total.toFixed(2)}</span>
                      </div>
                      <button
                  ...
                  <>
                    ...
                    <div className="p-6 bg-white border-t border-red-500/20 space-y-3">
                      ...
                      {checkoutSummaryOpen && (
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-black/50 font-bold">Subtotal</span>
                            <span className="text-black font-black">R$ {total.toFixed(2)}</span>
                          </div>
                          {couponCode && couponDiscount > 0 && (
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-black" style={{ color: "#145a2c" }}>
                            ...
                            (() => {
                              ...
                              const url = `https://wa.me/${
                                ...
                                formatWhatsAppMessage(items as unknown as CartItem[], {
                                  ...
                                  fulfillment,
                                  payment,
                                  paymentDetail: payment === "Cartão" ? cardType : "",
                                  changeFor: payment === "Dinheiro" ? changeFor : "",
                                  couponCode:
                                    couponCode && couponDiscount > 0
                                      ? String(couponCode).toUpperCase()
                                      : "",
                                  discount: couponCode && couponDiscount > 0 ? couponDiscount : 0,
                                })
                              )}`;
